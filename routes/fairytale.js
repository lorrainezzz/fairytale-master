...
router.findOne = (req, res) => {
    res.setHeader('Content-Type', 'application/json');
    Fairytale.find({ "id" : req.params.id },function(err, fairytale) {
        if (err)
            res.send('Fairy tale NOT Found!!');
        else
            res.send(JSON.stringify(fairytale,null,5));
    });
}
...
router.addFairytale = (req, res) => {
    ...
    if(!name){
        res.json({ message: 'Fairy tale name or author cannot be empty'} );
    }
    else if(!author){
        res.json({ message: 'Fairy tale name or author cannot be empty'} );
    }
    else{
        Fairytale.findOne({name:name},function (err, info) {
            if(info){
                res.json({ message: 'The fairytale is already exist',errmsg : err} );
                return;
            }
...
router.deleteFairytale = (req, res) => {
    Fairytale.findByIdAndRemove(req.params.id, function(err) {
        if (err)
            res.json({ message: 'Fairy tale NOT FOUND!'});
        else
            res.json({ message: 'Fairy tale Deleted!'});
    });
}
